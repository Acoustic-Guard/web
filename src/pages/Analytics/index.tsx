...
export default function AnalyticsPage() {
 ...
 const [showDatePicker, setShowDatePicker] = useState(false);
  ...
  const handleExportReport = () => {
    ...
    const headers = ['Date/Time', 'Threat Type', 'Latitude', 'Longitude', 'Confidence', 'Status'];

    const csvRows = data.history.map(incident => {
      const date = formatDate(incident.createdAt);
      const type = incident.type;
      const lat = incident.latitude !== null ? incident.latitude : '';
      const lng = incident.longitude !== null ? incident.longitude : '';
      const confidence = Math.round(incident.intensity * 100) + '%';
      const status = incident.status;
      return `"${date}","${type}","${lat}","${lng}","${confidence}","${status}"`;
    });
    ...
    link.href = url;
    link.setAttribute('download', `acoustic_guard_report_${timeRange}.csv`);
